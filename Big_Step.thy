...
AssignND: "v \<in> S \<Longrightarrow> (x ::= ND S, s) \<Down> s(x := v)" |
...
WhileTrue:
"\<lbrakk> bval b s\<^sub>1;  (c,s\<^sub>1) \<Down> s\<^sub>2;  (WHILE b DO c, s\<^sub>2) \<Down> s\<^sub>3 \<rbrakk>  
\<Longrightarrow> (WHILE b DO c, s\<^sub>1) \<Down> s\<^sub>3"
print_theorems
...
lemma "(IF b THEN SKIP ELSE SKIP, s) \<Down> t \<Longrightarrow> t = s"
by blast

text\<open>Rule inversion by hand via the ``cases'' method:\<close>

lemma assumes "(IF b THEN SKIP ELSE SKIP, s) \<Down> t"
shows "t = s"
proof-
...
proof
  assume "(c1;; c2;; c3, s) \<Down> s'"
  then obtain s1 s2 where
    c1: "(c1, s) \<Down> s1" and
    c2: "(c2, s1) \<Down> s2" and
    c3: "(c3, s2) \<Down> s'" by auto
  from c2 c3
  have "(c2;; c3, s1) \<Down> s'" by (rule Seq)
  with c1
  show "(c1;; (c2;; c3), s) \<Down> s'" by (rule Seq)
next
  \<comment> \<open>The other direction is analogous\<close>
  assume "(c1;; (c2;; c3), s) \<Down> s'"
  thus "(c1;; c2;; c3, s) \<Down> s'" by auto
qed
