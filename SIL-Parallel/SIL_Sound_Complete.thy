...
lemma sil_while_sound_ok:
  ...
  apply (induct n arbitrary: s rule: Nat.nat_less_induct, clarsimp)
    proof -
        fix na :: nat and sa :: "char list \<Rightarrow> int" and nat :: nat
        assume a1: "Q na sa"
        assume a2: "\<forall>m<na. \<forall>x. Q m x \<longrightarrow> (\<exists>t. (WHILE b DO c, x) \<Down> (t, True) \<and> Q 0 t)"
        obtain ii :: "(char list \<Rightarrow> int) \<Rightarrow> nat \<Rightarrow> char list \<Rightarrow> int" where
          f3: "\<forall>x0 x1. (\<exists>v2. (c, x0) \<Down> (v2, True) \<and> Q x1 v2) = ((c, x0) \<Down> (ii x0 x1, True) \<and> Q x1 (ii x0 x1))"
          by moura
        have f4: "(na < Suc 0) = (na = 0)"
          by simp
        have "\<forall>n f. \<not> Q (Suc n) f \<or> (c, f) \<Down> (ii f n, True) \<and> Q n (ii f n)"
          using f3 assms(5) by presburger
