...
assumes "\<And>n. \<turnstile>\<^sub>b \<langle>Q (Suc n)\<rangle> c  \<langle>OK (Q n)\<rangle>"
...
lemma sil_while_sound_er:
  assumes "\<And>n. \<turnstile>\<^sub>b \<langle>Q (Suc (Suc n))\<rangle> c \<langle>OK (Q (Suc n))\<rangle>"
  and "\<turnstile>\<^sub>b \<langle>Q 1\<rangle> c \<langle>ER (Q 0)\<rangle>"
